...
class StockPickingExtended(models.Model):
    ...
    #    if self.move_lines.ids and self.picking_type_id.id and self.picking_type_id.packaging_wise_split:
    #        for move in self.move_lines.filtered(lambda r:r.product_packaging_id and r.product_packaging_id.qty and r.product_id.tracking == 'lot'):
    ...
    #                    quantity -= quantity
    #
    #            package_move_lines = move.move_line_ids.filtered(
        ...
        for reserved_quant, quantity in quants:
            ...
            if to_update and float_compare(quantity, uom_quantity_back_to_product_uom, precision_digits=rounding) == 0:
                ...
                #TODO: CHANGES
                    ...
                    if len(list_values) > 0:
                # Move lines with serial tracked product_id cannot be to-update candidates. Delay the creation to speed up candidates search + create.
                        serial_move_line_vals.extend([self._prepare_move_line_vals(quantity=i, reserved_quant=reserved_quant) for i in list_values])
                else:
