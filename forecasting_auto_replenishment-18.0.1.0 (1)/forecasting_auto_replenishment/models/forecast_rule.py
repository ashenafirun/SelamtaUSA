...
class ForecastRule(models.Model):
    ...
    use_out_of_stock_days = fields.Boolean(
        string="Exclude Out Of Stock Days",
        help="If enabled, demand calculation excludes days when stock was zero.",
        default=False
    )
    ...
    def generate_product_forecast(self, products, period_from, period_to, history_days, sale_from,sale_to, forecast_vendor_id):
        ...
        if sales_map:
            ...
            for product in products:
                ...
                if self.use_out_of_stock_days:
                    out_of_stock_days = self.get_out_of_stock_days(
                        product, sale_from, sale_to, current_stock
                    )
                    effective_days = history_days - out_of_stock_days
                else:
                    effective_days = history_days

                avg_daily = round(sold_qty / effective_days, 2) if effective_days > 0 else 0
